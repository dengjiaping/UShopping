...
public class FilpperListvew extends ListView{
    ...
    public interface FilpperDeleteListener {
        //方法说明控制上下移动的位置
        void getMoveY(float moveX, float moveY);
        //移动位置的具体的接口
        void onFlipping(float xPosition, float yPosition, float apartX, float apartY);
        //最后的接口用来item最后的位置是否改变
        void restoreView(float x, float y, boolean tag);
    }
